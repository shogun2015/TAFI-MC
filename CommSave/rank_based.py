...
class Experience(object):
    ...
    def store(self, experience, init_thr):
        """
    ...
    def retrieve(self, indices):
        ...
        """
        priorities = [self.priority_queue.priority_queue[self.priority_queue.e2p[v]][0] for v in indices]
        return [self._experience[v] for v in indices]\
            , [np.mean(priorities), np.max(priorities), np.min(priorities), priorities]
    ...
    def sample(self, global_step):
        ...
        experience, m_p = self.retrieve(rank_e_id)
        return experience, w, rank_e_id, m_p
