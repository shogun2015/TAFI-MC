...
class MADDPG():
    def __init__(self, name, actor_lr, critic_lr, layer_norm=True, nb_actions=1, nb_other_aciton=3,
        ...
        rem_action_input = tf.placeholder(shape=[None, nb_actions], dtype=tf.float32)
        ...
        self.rem_action = rem_action_input
        ...
        self.actor_loss = tf.reduce_mean(tf.square(self.action_output - self.rem_action))  # reduce_mean 为求均值，即为期望
        self.actor_loss_error_value = tf.square(self.action_output - self.rem_action)
    ...
    def train_actor(self, state, other_action, rem_actions, sess, summary_writer, lr):
        self.count += 1
        self.actor_lr = lr
        summary_writer.add_summary(
            sess.run(self.actor_loss_op,
                     {self.state_input: state, self.other_action_input: other_action, self.rem_action: rem_actions}),
            self.count)
    ...
    def actor_loss_error(self, action, rem_actions, state_input, sess):
        return sess.run(self.actor_loss_error_value, {self.action_input: action, self.rem_action: rem_actions, self.state_input: state_input})
