...
class ReplayBuffer(object):
    ...
    def getBatch(self, batch_size):
        ...
        batch, w, e_id, m_p = self.replay_memory.sample(self.num_experiences)
        self.e_id = e_id
        self.w_id = w
        '''#state t
        ...
        return batch, self.w_id, self.e_id, m_p

    def size(self):
        return self.buffer_size

    def add(self, state, action, reward, next_state, done, init_thr):  # add(self, state, next_state, action, reward, done):
        ...
        if self.rand_s:
            ...
            self.replay_memory.store(new_experience, init_thr)
