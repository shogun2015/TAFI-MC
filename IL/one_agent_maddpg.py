...
def parse_args():
    ...
    parser.add_argument("--csv_name", type=str, default="test", help="name of the experiment")  # 实验名
        ...
        for i in range(args.benchmark_iters):
            for lane in range(4):
                for ind, veh in enumerate(env.veh_info[lane]):
                    ...
                    env.step(lane, ind, agent1_action[0][0], thr=args.thr)  # 环境根据输入的动作返回下一时刻的状态和奖励
...
def train():
    ...
    model_count = 0
    txt_writer = None
    txt_writer = open("priority_recorder.txt", "w")
        ...
        for i in range(20000):
            ...
            if count_n > args.learn_start:
                ...
                for s_t in range(args.itr):
                    if args.seq_data:
                        m_p = train_agent_seq(agent1_ddpg, agent1_ddpg_target, agent1_memory_seq,
                                              agent1_actor_target_update, agent1_critic_target_update, sess,
                                              summary_writer,
                                              args)
                        if txt_writer and isinstance(m_p, float):
                            txt_writer.write("%0.5f\n" % m_p)
                    else:
...
def test():
    ...
    for thr in thrs:
        for density in densities:
            args.exp_name = "train_only_RL_model_%s_0424" % (density)
            ...
            modes = [-1, 3, 6, 9]  # -1表示纯模型，thr表示结合规则
            for mode in modes:
                for mat_path in mat_paths:
                    ...
                    for l in lens:
                        ...
                        exp_log = "%s_%s_%s" % (args.exp_name, str(mode), mat_path.split(".")[0].split("_")[-1])
